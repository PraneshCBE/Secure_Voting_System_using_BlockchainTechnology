...
router.post('/',async (req,res)=>{
    const voter = new Voter({
        ...
        otpVerfied:req.body.otpVerfied
    })
    try {
...
router.patch('/:aadharNumber',getVoterDetails,async (req,res)=>{
        res.voter.otpVerfied = true
        console.log(res.voter.otpVerfied)
    try {
        const updateVoter = await res.voter.save()
         res.status(201).json(updateVoter)
